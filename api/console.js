...
async function readBlob(prefix){const b=await latest(prefix);if(!b)return null;const result=await get(b.url,{access:'private'});if(!result||!result.stream)return null;return new Response(result.stream).json()}
